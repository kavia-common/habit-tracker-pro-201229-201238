...
// PUBLIC_INTERFACE
export function aggregateWeekly({ dayKeys, checkins, totalHabits }) {
  /**
   * Aggregate completion for a week (7 days) as a series:
   * [{ date, completedCount, totalHabits, completionRate }]
   */
  const keys = dayKeys && dayKeys.length ? dayKeys : getLastNDaysKeys(7);
